...
if(activeSourceFilter==="mine"){activeSourceFilter="";try{localStorage.removeItem("getlink:filter-source");}catch{}}
let tableSourceSort="";
let libraryQuery="";
let libraryCache=[];
...
try{localStorage.removeItem("getlink:view-mode");}catch{}
...
const WORK_ICON_PATHS={
  ...
  "news":'<path d="M4 5h16v14H4z"/><path d="M8 9h8M8 13h5M15 13h1M8 16h8"/>',
...
function newsTopicButtons(){
  return Object.entries(NEWS_TOPIC_LABELS).map(([key,label])=>
    '<button class="news-filter-chip '+(newsTopic===key?"active":"")+'" data-news-topic="'+escapeAttr(key)+'" type="button" aria-pressed="'+(newsTopic===key?"true":"false")+'">'+
      escapeHtml(label)+
    '</button>'
  ).join("");
}
...
function renderNewsDesktop(){
  const section=$("#userWorkNews");
  if(!section)return;
  section.hidden=userWorkDesktopScope!=="news";
...
function renderMobileNews(){
  const results=$("#mobileUserResults");
...
function refreshNewsViews(){
  if(isMobileUserWork()){
    if(mobileUserScope==="news")renderMobileNews();
  }else if(userWorkDesktopScope==="news"){
    renderNewsDesktop();
  }
}
...
function setNewsTopic(key){
  if(!Object.prototype.hasOwnProperty.call(NEWS_TOPIC_LABELS,key))key="latest";
  if(newsTopic===key&&newsItems.length)return;
  newsTopic=key;
  newsItems=[];
  newsError="";
...
function newsRenderQuickContent(value,fallback="",images=[],blocks=[]){
  const host=$("#newsQuickContent");
  if(!host)return;
  const cleanBlocks=Array.isArray(blocks)?blocks.filter(block=>block&&typeof block==="object"):[];
  ...
  paragraphs.forEach((text,index)=>{
    ...
    const shouldInsert=index===0||((index+1)%3===0);
    if(shouldInsert&&imageIndex<pictures.length){
      parts.push('<figure class="news-quick-inline"><img src="'+escapeAttr(pictures[imageIndex++])+'" alt="" loading="lazy" decoding="async" referrerpolicy="no-referrer"></figure>');
    }
  });
  while(imageIndex<pictures.length){
    parts.push('<figure class="news-quick-inline"><img src="'+escapeAttr(pictures[imageIndex++])+'" alt="" loading="lazy" decoding="async" referrerpolicy="no-referrer"></figure>');
  }
  host.innerHTML=parts.join("");
}
...
function isVinamilkRow(row){
  ...
  try{
    return new URL(String(row&&row.canonical_url||"")).hostname
      .toLowerCase().replace(/^www\./,"")==="vinamilk.com.vn";
  }catch{
    return false;
  }
}
...
function renderCategoryMenu(){
  ...
  let visibleLibrary=libraryCache.filter(row=>
    String(row.preference_state||"normal")!=="hidden"&&!isMineRow(row)
  );
  ...
  if(quickCurrent){
    const sourceLabel=activeSourceFilter==="bhx"?"BHX":(activeSourceFilter==="wm"?"WinMart":(activeSourceFilter==="go"?"GO!":(activeSourceFilter==="vinamilk"?"VNM":"Tất cả nguồn")));
...
function gridProductCard(row){
  ...
  return '<article class="grid-product product-card'+marketPackClass+catalogSourceDisplayClass(row)+' '+
    ...
    '" tabindex="0" data-url="'+escapeAttr(row.canonical_url)+'">'+
      ...
      '<div class="grid-product-body">'+
        ...
        '<div class="grid-product-bottom">'+
          ...
          '<small class="grid-source-tag '+escapeAttr(sourceKey)+'">'+escapeHtml(sourceLabel)+'</small>'+
...
function rowsBeforeSearch(){
  ...
  return memoBrowseRows(key,()=>{
    let products=libraryCache.filter(row=>!isMineRow(row));
...
function categoryBaseRows(){
  ...
  return memoBrowseRows(key,()=>{
    let rows=libraryCache.filter(row=>
      String(row.preference_state||"normal")!=="hidden"&&!isMineRow(row)
    );
...
const TABLE_SOURCE_CYCLE=["","bhx","wm","go","vinamilk"];
...
function sourceLogoMark(key){
  ...
  if(key==="vinamilk"){
    return '<span class="source-logo-mark source-logo-vinamilk" aria-hidden="true"><b class="source-logo-fallback">VNM</b></span>';
  }
  if(logos[key]){
  ...
  return '<span class="source-logo-mark source-logo-all" aria-hidden="true"><b>5</b></span>';
}
...
function renderSourceTabs(){
  ...
  if(!["","bhx","wm","go","vinamilk"].includes(activeSourceFilter)){
    activeSourceFilter="";
...
function filteredLibraryProducts(){
  ...
  return memoBrowseRows(key,()=>{
    ...
    return userWorkMarketSortRows(products);
  });
}
...
function mobileMergeTargetRow(){
  const row=findLibraryRow(mobileMergeTargetUrl);
  if(row&&isMineRow(row))return row;
  return null;
}

function mobileMergeTargetCanonicalId(){
  const row=mobileMergeTargetRow();
  return String(row&&row.canonical_product_id||"").trim();
}

function mobileMergeTargetMembers(){
  const target=mobileMergeTargetRow();
  if(!target)return [];
  const id=String(target.canonical_product_id||"").trim();
  if(!id)return [target];
...
function setMobileMergeTarget(row){
  ...
  if(!isMineRow(row)&&id){
    const own=libraryCache.find(item=>
      String(item&&item.canonical_product_id||"").trim()===id&&isMineRow(item)
    );
    if(!own)return false;
    row=own;
...
function mobileMergeSelectButton(row){
  ...
  const rowId=String(row&&row.canonical_product_id||"").trim();

  if(isMineRow(row)){
    const selected=Boolean(target&&canonical(target.canonical_url)===url);
    return '<button class="mobile-merge-select '+(selected?"selected":"")+'" '+
      'data-mobile-merge-target="'+escapeAttr(url)+'" type="button" aria-pressed="'+(selected?"true":"false")+'">'+
      (selected?"✓":"◎")+
    '</button>';
  }
...
function renderMobileMergePanel(){
  ...
  const target=mobileMergeTargetRow();
  const targetHost=$("#mobileMergeTarget");
  const hint=$("#mobileMergeHint");
  if(targetHost){
    targetHost.textContent=target
      ?canonicalDisplayName(target)
      :"Chọn hàng Tạp hóa làm sản phẩm đích";
  }
  if(hint){
...
function supportedSourceUrl(raw){
  try{
    ...
    return host==="bachhoaxanh.com"||host==="winmart.vn"||host==="sieuthi-go.vn"||host==="vinamilk.com.vn";
  }catch{
    return false;
  }
}
