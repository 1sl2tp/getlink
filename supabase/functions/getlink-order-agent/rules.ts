...
function quantityFromTail(text:string): {name:string;quantity:number}|null {
...
export function parseFastCommand(text:unknown): ParsedIntent|null {
  ...
  if(qty)return {
    intent:"add_item",raw_product_text:qty.name,quantity:qty.quantity,unit_hint:null,attributes:{},line_note:"",
    reference_target:null,needs_clarification:false,
  };
  return null;
}
