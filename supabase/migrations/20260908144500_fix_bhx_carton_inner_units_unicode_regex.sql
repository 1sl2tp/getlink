with parsed as (
  select
    l.canonical_url,
    (regexp_match(l.name, '^Thùng[[:space:]]+([0-9]+)[[:space:]]+(ly|tô|bình)([[:space:]]|$)', 'i'))[1]::int as qty,
    lower((regexp_match(l.name, '^Thùng[[:space:]]+([0-9]+)[[:space:]]+(ly|tô|bình)([[:space:]]|$)', 'i'))[2]) as unit_key
  ...
  where l.source='Bách Hóa XANH'
    ...
    and l.name ~* '^Thùng[[:space:]]+[0-9]+[[:space:]]+(ly|tô|bình)([[:space:]]|$)'
)
