...
-- Keep the operator note with the exact order-line snapshot. Catalog rows are not mutated.
...
begin
  if not exists (
    ...
    where conname='getlink_sales_order_items_line_note_check'
      and conrelid='public.getlink_sales_order_items'::regclass
  ) then
    alter table public.getlink_sales_order_items
      add constraint getlink_sales_order_items_line_note_check
      check (char_length(line_note) <= 160);
  end if;
end;
$$;
...
begin
  ...
  where btrim(coalesce(x.item->>'productCode',''))<>''
    ...
    and char_length(btrim(coalesce(x.item->>'lineNote','')))<=160;
  ...
  insert into public.getlink_sales_order_items(
    ...
    unit_price_vnd,bargain_price_vnd,unit_cost_vnd,source_key,line_note
  )
  select v_id,x.ord::integer,
         ...
         btrim(coalesce(x.item->>'lineNote',''))
