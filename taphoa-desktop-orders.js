(()=>{
  ...
  const state={status:"pending",query:"",timePreset:"today",from:"",to:"",source:"",sourceMode:"orders",orders:[],selectedId:"",customers:[],busy:false,statusText:""};
  ...
  function sourceDetailRows(sourceKey=state.source){
    const key=String(sourceKey||"").trim();
    if(!key)return[];
    const out=[];
    for(const order of baseRows()){
      for(const item of items(order)){
        if(String(item?.sourceId||"").trim()!==key)continue;
        const qty=Math.max(0,Number(item?.qty||0));
        const price=Math.max(0,Number(item?.price||0));
        const hasCost=Object.prototype.hasOwnProperty.call(item||{},"cost")&&Number.isFinite(Number(item?.cost));
        const cost=hasCost?Math.max(0,Number(item.cost||0)):0;
        out.push({
          orderId:String(order?.id||""),orderNo:order?.orderNo,customerName:String(order?.customerName||"Khách hàng"),orderedAt:order?.orderedAt||order?.submittedAt,
          productId:String(item?.productId||""),name:String(item?.name||"Sản phẩm"),qty,price,cost,hasCost,
          revenue:qty*price,costTotal:qty*cost,profit:qty*(price-cost)
        });
      }
    }
    return out;
  ...
  function sourceSummary(sourceKey=state.source){
    const rows=sourceDetailRows(sourceKey),orderIds=new Set(rows.map(row=>row.orderId).filter(Boolean));
    const hasCost=rows.length>0&&rows.every(row=>row.hasCost);
    return {
      key:String(sourceKey||""),orders:orderIds.size,lines:rows.length,qty:rows.reduce((sum,row)=>sum+row.qty,0),
      revenue:rows.reduce((sum,row)=>sum+row.revenue,0),
      cost:hasCost?rows.reduce((sum,row)=>sum+row.costTotal,0):null,
      profit:hasCost?rows.reduce((sum,row)=>sum+row.profit,0):null,
      hasCost
    };
  }

  function combinedSourceRows(sourceKey=state.source){
    const map=new Map();
    for(const row of sourceDetailRows(sourceKey)){
      const groupKey=String(row.productId||normalize(row.name)||row.name).trim().toLowerCase();
      const current=map.get(groupKey)||{key:groupKey,productId:row.productId,name:row.name,qty:0,revenue:0,costTotal:0,profit:0,hasCost:true,orderIds:new Set()};
      current.qty+=row.qty;current.revenue+=row.revenue;current.costTotal+=row.costTotal;current.profit+=row.profit;current.hasCost=current.hasCost&&row.hasCost;
      if(row.orderId)current.orderIds.add(row.orderId);
      map.set(groupKey,current);
    }
    return [...map.values()].map(row=>({...row,orders:row.orderIds.size})).sort((a,b)=>b.qty-a.qty||a.name.localeCompare(b.name,"vi"));
  }

  function stats(){
    const keys=new Set();
    for(const order of baseRows())for(const key of sources(order).length?sources(order):["__unknown__"])keys.add(key);
    return [...keys].map(key=>{
      if(key==="__unknown__"){
        const orders=baseRows().filter(order=>!sources(order).length);
        return{key,orders:orders.length,products:orders.reduce((sum,order)=>sum+products(order),0),revenue:orders.reduce((sum,order)=>sum+Number(order?.total||0),0)};
      }
      const summary=sourceSummary(key);
      return{key,orders:summary.orders,products:summary.qty,revenue:summary.revenue};
    }).sort((a,b)=>b.orders-a.orders||sourceLabel(a.key).localeCompare(sourceLabel(b.key),"vi"));
  }

  function renderSourceRail(){
    if(!slots?.left)return;if(!sourceNames.size)refreshSourceNames();const rows=baseRows();
    slots.left.innerHTML='<div class="taphoa-column-head"><strong>Theo nguồn</strong><small>'+rows.length+' đơn</small></div><div class="taphoa-left-list"><button type="button" class="taphoa-left-item '+(!state.source?'active':'')+'" data-td-order-source=""><span>Tất cả</span><small>'+rows.length+'</small></button>'+stats().map(x=>'<button type="button" class="taphoa-left-item '+(state.source===x.key?'active':'')+'" data-td-order-source="'+attr(x.key)+'"><span>'+esc(sourceLabel(x.key))+'</span><small>'+x.orders+' đơn · '+x.products+' SP</small></button>').join('')+'</div>';
  }

  function sourceReportMarkup(){
    if(!state.source||state.source==="__unknown__")return"";
    const summary=sourceSummary(state.source);
    return '<section class="taphoa-source-report-bar">'+
      '<div class="taphoa-source-report-copy"><strong>'+esc(sourceLabel(state.source))+'</strong><small>'+summary.orders+' đơn · '+summary.lines+' dòng · '+summary.qty+' sản phẩm · DT '+esc(money(summary.revenue))+(summary.hasCost?' · Lãi '+esc(money(summary.profit)):'')+'</small></div>'+
      '<div class="taphoa-source-report-actions">'+
        '<button type="button" data-td-source-mode="orders" class="'+(state.sourceMode==='orders'?'active':'')+'">Đơn</button>'+
        '<button type="button" data-td-source-mode="detail" class="'+(state.sourceMode==='detail'?'active':'')+'">Chi tiết</button>'+
        '<button type="button" data-td-source-mode="combined" class="'+(state.sourceMode==='combined'?'active':'')+'">Gộp</button>'+
        '<button type="button" data-td-source-share>Chia sẻ</button>'+
      '</div></section>';
  }
  ...
  function sourceDetailMarkup(){
    const rows=sourceDetailRows(state.source);
    return rows.length?rows.map((row,index)=>'<button type="button" class="taphoa-source-detail-row" data-td-order-id="'+attr(row.orderId)+'"><small>'+(index+1)+'</small><span><strong>'+esc(row.name)+'</strong><em>'+esc(row.customerName)+' · Đơn #'+esc(row.orderNo||row.orderId)+' · '+esc(dateTime(row.orderedAt))+'</em></span><b>'+row.qty+'</b><span>'+esc(money(row.revenue))+'</span></button>').join(''):'<div class="taphoa-empty">Chưa có dòng hàng của nguồn này.</div>';
  }
  function combinedSourceMarkup(){
    const rows=combinedSourceRows(state.source);
    return rows.length?rows.map((row,index)=>'<div class="taphoa-source-combined-row"><small>'+(index+1)+'</small><span><strong>'+esc(row.name)+'</strong><em>'+row.orders+' đơn</em></span><b>'+row.qty+'</b><span>'+esc(money(row.revenue))+'</span>'+(row.hasCost?'<span class="taphoa-source-profit">'+esc(money(row.profit))+'</span>':'')+'</div>').join(''):'<div class="taphoa-empty">Chưa có sản phẩm để gộp.</div>';
  }
  function orderBodyMarkup(rows){
    if(state.source&&state.sourceMode==="detail")return sourceDetailMarkup();
    if(state.source&&state.sourceMode==="combined")return combinedSourceMarkup();
    return rows.length?rows.map(rowMarkup).join(''):'<div class="taphoa-empty">Chưa có đơn phù hợp.</div>';
  }
  ...
  function updateSourceReportSlot(){const slot=slots?.master?.querySelector?.(".taphoa-source-report-slot");if(slot)slot.innerHTML=sourceReportMarkup()}
  function renderOrderRows(){if(!slots?.master)return;const rows=visibleRows();normalizeSelection(rows);const list=slots.master.querySelector(".taphoa-order-list");if(!list){renderOrderList();return}list.innerHTML=orderBodyMarkup(rows);updateSummary(rows);updateSourceReportSlot();renderOrderDetail(state.selectedId)}
  ...
  function sourceShareText(){
    const summary=sourceSummary(state.source),rows=combinedSourceRows(state.source);
    const header=sourceLabel(state.source)+' · '+statusLabel(state.status);
    const totals=summary.orders+' đơn · '+summary.qty+' sản phẩm · Doanh thu '+money(summary.revenue)+(summary.hasCost?' · Giá vốn '+money(summary.cost)+' · Lãi '+money(summary.profit):'');
    const productsText=rows.map((row,index)=>(index+1)+'. '+row.name+' · '+row.qty+' · '+money(row.revenue)).join('\n');
    return [header,totals,productsText].filter(Boolean).join('\n');
  }
  async function shareSourceReport(){
    if(!state.source||state.source==="__unknown__")return false;
    const text=sourceShareText();if(!text)return false;
    try{
      if(typeof navigator?.share==="function"){
        try{await navigator.share({text});state.statusText="Đã mở chia sẻ.";renderOrderDetail(state.selectedId);return true}catch(error){if(error?.name==="AbortError")return false}
      }
      if(navigator?.clipboard?.writeText){await navigator.clipboard.writeText(text);state.statusText="Đã sao chép báo cáo nguồn.";renderOrderDetail(state.selectedId);return true}
      state.statusText="Thiết bị chưa hỗ trợ chia sẻ hoặc sao chép.";renderOrderDetail(state.selectedId);return false;
    }catch(error){state.statusText=String(error?.message||error);renderOrderDetail(state.selectedId);return false}
  }
  ...
  async function batchCurrentOrders(kind){
    ...
    if(kind==="pending"&&state.status==="pending"){
      ...
      try{
        ...
        state.busy=false;state.statusText="";state.selectedId="";state.source="";state.sourceMode="orders";await refresh();return;
  ...
  function bind(){
    ...
    slots.left.addEventListener("click",e=>{const b=e.target.closest?.("[data-td-order-source]");if(!b)return;state.source=String(b.dataset.tdOrderSource||"");state.sourceMode="orders";renderSourceRail();renderOrderRows()});
    slots.master.addEventListener("click",e=>{
      const mode=e.target.closest?.("[data-td-source-mode]");if(mode){state.sourceMode=String(mode.dataset.tdSourceMode||"orders");renderOrderRows();return}
      if(e.target.closest?.("[data-td-source-share]")){void shareSourceReport();return}
      const batch=e.target.closest?.("[data-td-order-batch]");if(batch){void batchCurrentOrders(String(batch.dataset.tdOrderBatch||""));return}
      const tab=e.target.closest?.("[data-td-order-status]");if(tab){state.status=String(tab.dataset.tdOrderStatus||"pending");state.source="";state.sourceMode="orders";state.selectedId="";slots.master.querySelectorAll("[data-td-order-status]").forEach(b=>b.classList.toggle("active",b===tab));renderSourceRail();renderOrderRows();return}
      const row=e.target.closest?.("[data-td-order-id]");if(row)selectOrder(String(row.dataset.tdOrderId||""))
    });
    ...
    slots.detail.addEventListener("change",e=>{const s=e.target.closest?.("[data-td-order-customer]");if(s)void changeCustomer(String(s.dataset.tdOrderId||""),String(s.value||""))});
  ...
  window.TaphoaDesktopOrders={mount,activate,deactivate,refresh,renderSourceRail,renderOrderList,renderOrderRows,renderOrderDetail,selectOrder,batchCurrentOrders,sourceSummary,sourceDetailRows,combinedSourceRows,shareSourceReport,actionMarkup,performAction:mutate,get state(){return state}};
})();
