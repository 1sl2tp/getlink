...

{
  ...
  assert.ok(searchKeyMatch&&stripCartonMatch&&stripLotMatch&&searchDisplayNameMatch&&productSearchKeyMatch&&matchesSearchMatch,"search helpers not found");
  const api=new Function(
...

{
  ...
  assert.match(css,/min-width:1240px!important/);
  assert.match(css,/\.xls-price-table:not\(\.supplier-table-mode\) \.xls-compact-only\{display:none!important\}/);
...

{
  ...
  assert.match(app,/const TABLE_SOURCE_CYCLE=\["","bhx","wm","go","vinamilk"\]/);
  assert.match(app,/function nextTableSourceFilter\(\)/);
...

{
  ...
  assert.match(html,/>Tên SP</);
  assert.match(html,/>Giá thùng</);
  assert.match(html,/>QC</);
  assert.match(html,/>Giá lẻ</);
  assert.match(html,/>Đơn vị lẻ</);
  assert.match(html,/>Mặc cả</);
  assert.match(html,/>Đánh giá</);
  assert.match(html,/>Nguồn</);
  assert.match(css,/Customer value-sized columns v25/);
  assert.match(css,/body\[data-app-role="user"\] #standardTableHead,[\s\S]*?#supplierTableHead\{[\s\S]*?display:none!important/);
  assert.match(app,/function userSourceCell\(row\)/);
  assert.match(app,/user-col-rating[\s\S]*?user-col-source/);
