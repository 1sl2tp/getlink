...
class NewsRssContractTest(unittest.TestCase):
    ...
    def test_news_ui_hides_publishers_and_mobile_is_two_columns(self):
        ...
        self.assertIn("news-card-time",card)
        self.assertIn("newsPublishedLabel",APP)
        self.assertIn("News QA v61",CSS)
    ...
    def test_news_detail_endpoint_is_sanitized_and_host_limited(self):
        ...
        self.assertIn('npm:@mozilla/readability@0.6.0',EDGE)
        self.assertIn('npm:linkedom@0.18.12',EDGE)
        self.assertIn("NEWS_ARTICLE_RULES",EDGE)
        self.assertIn("newsArticleRule",EDGE)
        self.assertIn("newsReadabilityRoot",EDGE)
        self.assertIn('"vnexpress.net"',EDGE)
        self.assertIn('"vietnamnet.vn"',EDGE)
        self.assertIn('"dantri.com.vn"',EDGE)
        self.assertIn('"tuoitre.vn"',EDGE)
        self.assertIn('"thanhnien.vn"',EDGE)
    ...
    def test_browser_keeps_ready_snapshot_while_refreshing(self):
        ...
        self.assertNotIn('apiFetch("/api/news?topic=',APP)
        self.assertIn('NEWS_SNAPSHOT_BASE_URL+encodeURIComponent(key)+".json?v="+bucket',APP)
        self.assertIn('news_snapshot_invalid_ready_item',APP)
